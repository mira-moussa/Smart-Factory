...
class Protocol1PacketHandler(object):
    ...
    def txPacket(self, port, txpacket):
        ...
        txpacket[total_packet_length - 1] = ~checksum & 0xFF  #checksum

        #print ("[TxPacket] %r" % txpacket)             #motawaka3 yb3atle inst_packet
    ...
    def read10ByteTxRx(self, port, dxl_id, address):
        data, result, error = self.readTxRx(port, dxl_id, address, 4)
        data_read = DXL_MAKEWORD1(data[0], data[1]) if (result == COMM_SUCCESS) else 0
        return data_read, result, error, data
    ...
    def write4ByteTxRx(self, port, dxl_id, address, pos, speed):
        data_write = [DXL_LOBYTE(pos),
                      DXL_HIBYTE(pos),
                      DXL_LOBYTE(speed),
                      DXL_HIBYTE(speed)]
        #print(data_write)                   #3ayza a4of bye3ml eh
    ...
    def syncWriteTxOnly(self, port, start_address, data_length, param, param_length):
        ...
        txpacket[PKT_ID] = BROADCAST_ID       #254=0xfe
