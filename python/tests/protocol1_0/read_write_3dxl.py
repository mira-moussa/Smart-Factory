...
while 1:
    ...
    if dxl_comm_result != COMM_SUCCESS:
     ...
     # Read dxl1 present speed
    dxl_present_speed, dxl_comm_result, dxl_error = packetHandler.read2ByteTxRx(portHandler, DXL1_ID,
    ...
    print("[ID:%03d] GoalPos:%03d  PresSpeed:%03d" % (DXL1_ID, dxl_goal_speed[index_speed], dxl_present_speed))

    # Read dxl2 present speed
    dxl_present_speed, dxl_comm_result, dxl_error = packetHandler.read2ByteTxRx(portHandler, DXL2_ID,
                                                                                ADDR_RX_PRESENT_SPEED)
    if dxl_comm_result != COMM_SUCCESS:
        print("%s" % packetHandler.getTxRxResult(dxl_comm_result))
    elif dxl_error != 0:
    ...
    print("[ID:%03d] GoalPos:%03d  PresSpeed:%03d" % (DXL2_ID, dxl_goal_speed[index_speed], dxl_present_speed))

    # Read dxl3 present speed
    dxl_present_speed, dxl_comm_result, dxl_error = packetHandler.read2ByteTxRx(portHandler, DXL3_ID,
                                                                                   ADDR_RX_PRESENT_SPEED)
    if dxl_comm_result != COMM_SUCCESS:
        print("%s" % packetHandler.getTxRxResult(dxl_comm_result))
    elif dxl_error != 0:
    ...
    print("[ID:%03d] GoalPos:%03d  PresSpeed:%03d" % (DXL3_ID, dxl_goal_speed[index_speed], dxl_present_speed))
    time.sleep(0.1)
