...
DXL_ZIKAS_ID = [4, 5, 6]  # used dxl ID
...
for i in range(3):
    ...
    dxl_comm_result, dxl_error = packetHandler.write1ByteTxRx(portHandler, DXL_MORAD_ID[i], ADDR_RX_TORQUE_ENABLE,
                                                              TORQUE_ENABLE)
    if dxl_comm_result != COMM_SUCCESS:
        ...
        print("Dynamixel:%03d has been successfully connected", DXL_MORAD_ID[i])

for i in range(3):
    # Enable Dynamixel Torque
    dxl_comm_result, dxl_error = packetHandler.write1ByteTxRx(portHandler, DXL_ZIKAS_ID[i], ADDR_RX_TORQUE_ENABLE,
                                                              TORQUE_ENABLE)
    if dxl_comm_result != COMM_SUCCESS:
        print("%s" % packetHandler.getTxRxResult(dxl_comm_result))
    elif dxl_error != 0:
        ...
        print("Dynamixel:%03d has been successfully connected", DXL_ZIKAS_ID[i])
...
for k in range(8): # zikas_traj
    ...
    [position, dxl_goal_sp, acc, posPlt, SpeedPlt] = traj(np.rad2deg(theta0), np.rad2deg(DXL_GOAL_POSITION), t, N,
                                                          k)
