...
def speed(a1, a2, a3, t, N):
    ...
    for i in range(0, N):
        ...
        theta_dot_t[i][0] = theta_dot_t_i * (np.pi / 180)  # speed rad/s
    return theta_dot_t
...
def traj(theta0, thetaf, t, N):
    profile_position = np.empty((N, 1))
    profile_speed = np.empty((N, 1))
    profile_acc = np.empty((N, 1))
    dt = t / (N - 1)
    time = np.zeros(shape=(N, 1))
    for r in range(0, N):            # Create time column vector for plotting
        a = r * dt
        time[r][0] = a
    for i in range(3):
        [a0, a1, a2, a3] = func(theta0[i], thetaf[i], t)
        theta1 = position(a0, a1, a2, a3, t, N)      # positions
        theta1_speed = speed(a1, a2, a3, t, N)       # speeds
        theta1_speed = theta1_speed
        theta1_acc = acceleration(a2, a3, t, N)      # acceleration
    ...
    return profile_position, profile_speed, profile_acc
